...
BOOST_AUTO_TEST_SUITE(rtscm_test)
...
BOOST_AUTO_TEST_CASE(m_n_size_assignment) {
	...
	auto m1 = make_rtscm(hana::type_c<double>, m1_sz, row_major_c);
	...
	auto m2 = make_rtscm(hana::type_c<double>, make_matrix_size(test::mat_g_m, test::mat_g_n), row_major_c);
	auto m2_m = m(size(m1));
	auto m2_n = n(size(m1));
